function getWeather(){
	try{
		var datasourceDetails = JSON.parse(""+$s.getDatasourceDetails("webSvcTest"));
		var baseUrl = datasourceDetails.datasourceProperties.baseUrl;
		var functionDetails = JSON.parse(""+$s.getFunctionDetails(datasourceDetails.datasourceProperties.id, "getWeather"));
